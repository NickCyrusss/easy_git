...
- **Git operations:** commit, create and switch branches, create tags, manual fetch from the toolbar, fast-forward-only pull, push (first push of a branch without an upstream publishes it to `origin` and sets tracking), cherry-pick, merge, revert, soft/mixed/hard reset, clone, initialization, branch deletion, and force push with an explicit lease.
...
- **Silent monitoring:** the active repository is checked about once per second in an independent background task. Local file/index, commit, ref and stash changes refresh the view only when detected. Monitoring leaves actions enabled and keeps the status bar, draft and preview intact; it pauses during operations, dialogs and button/drag interactions; typing a commit draft remains supported. Background failures retry silently.
- **Silent fetch:** the active repository fetches all configured remotes once when first monitored, then every 5 minutes while active. Fetch scheduling pauses during operations, dialogs and button/drag interactions; an already running fetch may finish after switching tabs. Fetch runs independently of local monitoring, keeps actions enabled, and does not show loading/errors or alter the draft, working tree or local branch. Failures wait until the next interval; fetched refs appear through silent monitoring. The toolbar Fetch button remains available for immediate fetching.
- **Refresh on repository switch:** selecting a repository tab refreshes its local status and commit graph. Busy tasks finish first; commit/stash previews, file selection, and commit drafts are preserved, and open working-file diffs are updated. The switch refresh reads local Git data; the separate silent-fetch schedule may also run when the tab becomes active.
...
Right-click a file in a commit or Stash file list and select **Apply changes to working file** to apply that file's diff to the current working tree. You do not need to open its diff first. The operation keeps the index unchanged and does not create a commit; a Stash is retained. It applies the patch rather than restoring the entire historical file. Binary diffs and patches that do not match the current file are rejected with an error.
...
Git authentication uses your existing SSH configuration or credential helper. The application does not store Git credentials or prompt for passwords. Git HTTP(S) operations inherit the process's proxy environment; SSH uses your SSH configuration.
...
| `git_operations` | Cherry-pick, merge, revert, reset modes, stash handling, applying a commit file diff while preserving the index, conflict continuation/abort/skip, and discard protections |
...
- Commit drafts, file selections, and panel layout last only for the current session. The active repository checks for local changes about once per second and updates silently. External file saves/deletions and staged-content changes refresh the corresponding working-file preview, clearing stale line selections only when its diff changes. Commit/stash previews stay pinned to their revisions; unstaged edits do not replace staged previews. Refresh remains available for immediate reloads.

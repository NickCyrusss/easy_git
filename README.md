...
- **Automatic fetch:** switching repository tabs fetches the selected repository once when the application is in the foreground; busy repositories wait until their current operation finishes. Automatic fetch preserves the current preview, file selection, and commit draft; failures appear in the status bar. There is no periodic fetch of open repositories.

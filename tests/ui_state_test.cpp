...
int main() {
    ...
    try {
        ...
        {
            auto source = base/"fetch-source";
            fs::create_directory(source); eg::Git upstream(source.string());
            upstream.checked({"init","--initial-branch=main"});
            upstream.checked({"config","user.name","Fetch Test"});
            upstream.checked({"config","user.email","fetch@example.invalid"});
            upstream.checked({"config","commit.gpgsign","false"});
            std::ofstream(source/"file.txt") << "initial\n";
            upstream.checked({"add","."}); upstream.commit("Initial");
            App fetching_app;
            auto wait_fetch = [&] {
                auto deadline = std::chrono::steady_clock::now()+std::chrono::seconds(10);
                while (fetching_app.busy()) {
                    for (auto& tab : fetching_app.tabs) tab->poll();
                    require(std::chrono::steady_clock::now()<deadline,"Automatic fetch did not finish");
                    std::this_thread::sleep_for(std::chrono::milliseconds(5));
                }
            };
            for (const char* name : {"fetch-a","fetch-b"}) {
                auto target = base/name;
                upstream.checked({"clone",source.string(),target.string()});
                fetching_app.add_tab().load(target.string());
            }
            wait_fetch();
            auto& first = *fetching_app.tabs[0]; auto& second = *fetching_app.tabs[1];
            auto old_tip = upstream.checked({"rev-parse","HEAD"});
            auto advance = [&] {
                std::ofstream(source/"file.txt",std::ios::app) << "new line\n";
                upstream.checked({"add","."}); upstream.commit("Advance remote");
                return upstream.checked({"rev-parse","HEAD"});
            };
            auto tip = [&](RepoTab& tab) { return eg::Git(tab.repo.root).checked({"rev-parse","origin/main"}); };
            auto new_tip = advance();
            first.workspace = false; first.show_diff = true; first.selected_file = "file.txt";
            first.selected_commit = "kept commit"; first.set_detail("kept preview");
            first.changed_files = eg::Git(first.repo.root).commit_files(first.repo.commits.front());
            first.rebuild_file_lists(); first.file_selection[2].insert("file.txt"); first.selected_lines.insert(0);
            snprintf(first.message,sizeof(first.message),"kept draft");
            fetching_app.active = first.id;
            fetching_app.schedule_fetch(false);
            require(!fetching_app.busy() && tip(first)==old_tip,"Background window initiated fetch");
            fetching_app.schedule_fetch(true); wait_fetch();
            require(tip(first)==new_tip && tip(second)==old_tip,"Switch fetch missed the selected repository or fetched an inactive repository");
            require(!first.workspace && first.show_diff && first.selected_file=="file.txt" && first.selected_commit=="kept commit" &&
                    first.detail=="kept preview" && first.file_selection[2].count("file.txt") && first.selected_lines.count(0) && std::string(first.message)=="kept draft","Automatic fetch reset the current view or draft");
            require(std::any_of(first.repo.refs.begin(),first.repo.refs.end(),[&](const auto& ref) {
                return ref.full=="refs/remotes/origin/main" && ref.id==new_tip.substr(0,new_tip.size()-1);
            }),"Fetch did not refresh remote refs in the UI");
            fetching_app.schedule_fetch(true);
            require(!fetching_app.busy(),"Automatic fetch repeated without a repository switch");
            auto switched_tip = advance();
            fetching_app.active = second.id;
            second.load(second.repo.root); // A switch during an existing job must remain pending.
            fetching_app.schedule_fetch(true); wait_fetch();
            require(second.fetch_pending && tip(second)==old_tip,"Switch fetch overlapped an existing job or was lost");
            fetching_app.schedule_fetch(true); wait_fetch();
            require(tip(second)==switched_tip && tip(first)==new_tip,"Switch did not fetch only the selected repository");
            auto saved_time = GImGui->Time;
            GImGui->Time += 3600;
            fetching_app.schedule_fetch(false);
            fetching_app.schedule_fetch(true);
            GImGui->Time = saved_time;
            require(!fetching_app.busy() && tip(first)==new_tip,"Elapsed time or foreground return triggered a periodic fetch");
            eg::Git(first.repo.root).checked({"remote","set-url","origin",(base/"missing-remote").string()});
            fetching_app.active = first.id;
            fetching_app.schedule_fetch(true); wait_fetch();
            require(first.error.empty() && first.status.find("Auto-fetch failed:")==0,"Auto-fetch failure was hidden or opened a blocking error");
            fetching_app.schedule_fetch(true);
            require(!fetching_app.busy(),"Failed automatic fetch retried without a repository switch");
            fetching_app.shutdown();
        }

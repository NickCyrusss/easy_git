...
- 切换仓库标签时，在程序前台自动 Fetch 当前仓库一次。仓库忙碌时等待当前任务完成，自动更新保留预览、文件选择和提交草稿，失败信息显示在状态栏；不执行定时 Fetch。

...
- 顶部 **Pull / Push** 后的 **Stash** 按钮一键保存当前仓库全部已暂存、未暂存和未跟踪改动（不包含被忽略文件），名称取自 **Commit summary**；摘要为空时使用 `Saved from easy git`，保留原提交草稿。没有改动、尚无首次提交或操作进行中时不可用。

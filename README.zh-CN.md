...
本文档描述当前源码。静默监控、自动 Fetch 和应用历史文件 Diff 是 v0.1.3 安装包之后新增的功能，使用这些功能请从源码构建。
...
- 窗口尺寸、已打开仓库、当前仓库、主题和 AI 设置自动覆盖保存到 **`~/.easy_git`**（JSON）。采用临时文件替换，权限为 **0600**；保存失败会提示错误并自动重试，只有写入成功才视为已保存；API Key 明文保存在这个仅当前用户可读写的文件中。启动时恢复仓库和设置，关闭的仓库不再恢复；不存在的仓库会显示错误并保留路径，便于下次重试。
...
- 在提交或 Stash 的文件列表中右键选择 **Apply changes to working file**，将该文件的 Diff 应用到当前工作区，无需先打开 Diff。操作保留暂存区、不创建提交，Stash 也会保留；应用的是补丁，不是用历史版本覆盖整个文件。二进制 Diff 或与当前文件不匹配的补丁会被拒绝并显示错误。
...
- 当前仓库约每秒静默后台检查一次，仅在本地文件／暂存区、提交、引用或 Stash 变化时更新。检查不禁用按钮、不修改状态栏、不弹窗，保留草稿和预览；Git 操作、模态窗口及按钮／拖动交互期间暂缓，输入提交草稿时仍可后台检查，失败后静默重试。
- 当前仓库首次进入监控时静默 Fetch 一次，此后在当前标签中每 5 分钟 Fetch 所有配置的远端。Git 操作、弹窗及按钮／拖动交互期间暂缓发起 Fetch，已开始的 Fetch 可在切换标签后完成。Fetch 独立于本地变化检查，不禁用按钮、不显示加载或错误、不改草稿、工作区和本地分支；失败等待下个间隔重试，完成后由静默监控更新远端引用。也可使用顶部 Fetch 按钮立即更新。
- 切换仓库标签时自动刷新本地状态和提交图，仓库忙碌时等待当前任务完成；保留提交／Stash 预览、文件选择和提交草稿，并更新打开的工作区文件 Diff。切换刷新本身只读取本地 Git 数据；标签激活后，独立的静默 Fetch 也可能按计划执行。
...
首次提交需要已有 Git 作者配置；网络认证使用已有 Git credential helper / SSH 配置。程序不保存 Git 凭据，不提示输入密码；AI API Key 按上文保存到本地配置。HTTP(S) 远程操作继承启动环境中的代理变量；SSH 代理仍由 SSH 配置决定。
...
`git_operations` 验证 Cherry-pick / Merge / Revert、合并提交主线选择、三种 Reset、应用提交中单个文件的 Diff 并保留暂存区、Stash 只读预览与未跟踪文件、恢复暂存状态、过期删除保护，以及真实冲突的继续、中止和跳过、外部 Rebase 中止、独立 Worktree 的操作状态，以及 Discard 对部分暂存、删除、重命名、特殊路径、符号链接和过期状态的处理，以及批量 Discard 的整组选中文件检查和暂存内容保留。
...
- 窗口尺寸、仓库、当前仓库、主题和 AI 设置持久化；提交草稿、文件选择和面板布局仅保留在当前会话，退出后不恢复；当前仓库约每秒静默检查本地变化，外部文件保存／删除及暂存内容变化会更新对应工作区 Diff，仅在 Diff 变化时清除旧行选区；Commit／Stash 预览固定在所选版本，未暂存修改不替换已暂存预览，也可手动 Refresh 立即重载。Clone、初始化、删除分支及带 lease 的强制推送已提供界面入口。
